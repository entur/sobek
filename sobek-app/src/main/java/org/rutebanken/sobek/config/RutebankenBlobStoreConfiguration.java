...
import org.rutebanken.sobek.service.BlobStoreService;
import org.rutebanken.sobek.service.RutebankenBlobStoreService;
 ...
 * </ul>
 * <p>
 * Sobek also has its own storage abstraction, {@link org.rutebanken.sobek.service.BlobStoreService}. An adapter is
 * provided ({@link org.rutebanken.sobek.service.RutebankenBlobStoreService}) which will be used if the
 * <code>rutebanken-blobstore</code> profile is active. The adapted blob stores complement Sobek's own implementation
 * in non-conflicting manner, meaning that of Sobek's blobstore profiles (<code>gcs-blobstore</code>, <code>local-blobstore</code>)
 ...
 * this <code>rutebanken-helpers adaptation</code> is not needed for GCS support within Sobek but for the other
 * implementations it provides, in practice these do not clash. If the GCS implementation is ever to be taken into use,
 * {@link org.rutebanken.sobek.service.GcsBlobStoreService} first needs to be adapted directly, and additional feature

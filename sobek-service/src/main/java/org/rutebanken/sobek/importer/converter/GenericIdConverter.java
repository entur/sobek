...
public class GenericIdConverter<X extends org.rutebanken.netex.model.DataManagedObjectStructure> {
    ...
    private final Class<? extends DataManagedObjectStructure> sobekType;

    public GenericIdConverter(ValidPrefixList validPrefixList, NetexIdHelper netexIdHelper, GenericEntityInVersionRepository genericEntityInVersionRepository, Class<? extends DataManagedObjectStructure> sobekType) {
    ...
    public X convertIncomingId(X object) {
        ...
        if(validPrefixList.isValidPrefixForType(netexIdHelper.extractIdPrefix(object.getId()), sobekType)) {
        ...
        try {
            var existingObjectId = genericEntityInVersionRepository.findByKeyValue(CustomKeyValueTypes.ORIGINAL_ID_KEY, Set.of(object.getId()), sobekType);

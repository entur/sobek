...
import org.rutebanken.netex.model.SchematicMapsInFrame_RelStructure;
...
@Component
public class SchematicMapImportHandler {

    private static final Logger logger = LoggerFactory.getLogger(SchematicMapImportHandler.class);
    ...
    public void handleSchematicMaps(ResourceFrame netexResourceFrame, ImportParams importParams, AtomicInteger schematicMapsCounter, ResourceFrame responseResourceframe) {
        ...
        if (publicationDeliveryHelper.hasSchematicMaps(netexResourceFrame)) {
            var originalSchematicMaps = netexResourceFrame.getSchematicMaps().getSchematicMap();
            logger.info("Publication delivery contains {} deck plans for import.", originalSchematicMaps.size());

            logger.info("About to check if incoming deck plans have previously been imported with the same id");
            var originalWithMappedIds = originalSchematicMaps.stream()
                    .map(schematicMapIdConverter::convertIncomingId)
                    .toList();
            ...
            responseResourceframe.withSchematicMaps(
                    new SchematicMapsInFrame_RelStructure()
                            .withSchematicMap(importedSchematicMaps));

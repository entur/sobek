...
package org.rutebanken.sobek.importer.modifier;
...
public class WordsRemover {
  ...
  public WordsRemover(List<String> words) {
    for (String word : words) {
      patterns.add(
        Pattern.compile("\\(" + word + "\\)\\s*$", Pattern.CASE_INSENSITIVE)
      );
      patterns.add(
        Pattern.compile("\\[" + word + "\\]\\s*$", Pattern.CASE_INSENSITIVE)
      );
      patterns.add(
        Pattern.compile(",\\s" + word + "\\s*$", Pattern.CASE_INSENSITIVE)
      );
    }
    logger.info(
      "Patterns: {}",
      patterns.stream().map(pattern -> "\"" + pattern + "\"").collect(toList())
    );
  }

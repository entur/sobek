...
import static org.rutebanken.sobek.service.TagCreator.SUPPORTED_TAGGABLE_TYPES;
...
public class TagKeyValuesMapper {
    ...
    public void mapTagsToProperties(org.rutebanken.sobek.model.DataManagedObjectStructure sobekEntity, DataManagedObjectStructure netexEntity) {

        if(sobekEntity.getNetexId() == null) {
            logger.trace("ID reference is null. Cannot continue mapping tags");
            return;
        }

        if(!SUPPORTED_TAGGABLE_TYPES.contains(sobekEntity.getClass())) {
            logger.trace("Ignoring tag mapping for class {} as it's not supported", sobekEntity.getClass());
            return;
        }

        Set<Tag> tags = tagRepository.findByIdReference(sobekEntity.getNetexId());

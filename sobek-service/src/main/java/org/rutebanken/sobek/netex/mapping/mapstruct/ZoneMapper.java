...
public interface ZoneMapper {
    ...
    @AfterMapping
    default void afterMapToSobek(Zone_VersionStructure source,
                                 @MappingTarget org.rutebanken.sobek.model.Zone_VersionStructure target,
                                 @Context MappingContext context) {
        if(target != null) {
            context.getDataManagedObjectStructureMapper().afterMappingToSobek(source, target, context);
            Point point = context.getSimplePointMapper().simplePointToPoint(source.getCentroid(), context);
            target.setCentroid(point);
        }
    }

    @AfterMapping
    default void afterMapToNetex(org.rutebanken.sobek.model.Zone_VersionStructure source,
        ...
        if(target != null) {
            ...
            target.setCentroid(context.getSimplePointMapper().pointToSimplePoint(source.getCentroid(), context));
        }
    }
    ...
    @Mapping(target = "version", source = "version", qualifiedByName = "versionToSobekZ")
    @Mapping(target = "keyValues", ignore = true) // Handle in AfterMapping
    @Mapping(target = "centroid", ignore = true) // Handle in AfterMapping
    @Mapping(target = "polygon", source = "polygon", qualifiedByName = "polygonTypeToPolygonZ")
    ...
    @Mapping(target = "centroid", ignore = true) // Handle in AfterMapping
    @Mapping(target = "polygon", source = "polygon", qualifiedByName = "polygonToPolygonTypeZ")
    @Mapping(target = "members", ignore = true) // Ignore for now TODO implement
    @interface ToNetexMappings {
    }

    @Named("polygonTypeToPolygonZ")
    default Polygon polygonTypeToPolygonZ(PolygonType polygonType, @Context MappingContext context) {
        return context.getPolygonMapper().polygonTypeToPolygon(polygonType);
    }

    @Named("polygonToPolygonTypeZ")
    default PolygonType polygonToPolygonTypeZ(Polygon polygon, @Context MappingContext context) {
